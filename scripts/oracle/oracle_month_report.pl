...
sub insert_table
{
    ...
    if($type eq "tablespace")
    {
        ...
        my $file_id = $ref->[3];
        my $file_name = $ref->[4];
        $cmd = "insert into oracle_tablespace_month_report(oracle_ip,date,tablespace_name,file_id,file_name,avg_free,noval) values('$oracle_ip','$date_now_str','$tablespace_name',$file_id,'$file_name',$avg,$nan_num)";
    }
    elsif($type eq "diskgroup")
    {
        my $diskgroup_id = $ref->[2];
        my $diskgroup_name = $ref->[3];
        $cmd = "insert into oracle_diskgroup_month_report(oracle_ip,date,diskgroup_id,diskgroup_name,avg_free,noval) values('$oracle_ip','$date_now_str',$diskgroup_id,'$diskgroup_name',$avg,$nan_num)";
    }
    elsif($type eq "disk")
    {
        my $disk_name = $ref->[2];
        my $disk_path = $ref->[3];
        $cmd = "insert into oracle_disk_month_report(oracle_ip,date,disk_name,disk_path,avg_free,noval) values('$oracle_ip','$date_now_str','$disk_name','$disk_path',$avg,$nan_num)";
    }
