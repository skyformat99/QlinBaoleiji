...
{
#	my $status = &stop_process($process_name,$val->[0],$val->[1]);
	my $status = &stop_process_parent($process_name,$val->[0],$val->[1]);
...
sub stop_process_parent
{
	my($name,$cmd,$port) = @_;
	if($port != 0)
	{
		my $result = `/usr/sbin/lsof -R -n -i:$port`;
		if($result ne "")
		{
			my %pid_arr;
            my %ppid_arr;
			...
			{
				my($pid,$ppid) = (split /\s+/,$line)[1,2];

				unless($pid =~ /\d+/ && $ppid =~ /\d+/) {next;}
				unless(exists $pid_arr{$pid})
				{
					$pid_arr{$pid} = $ppid;
				}
                
                unless(exists $ppid_arr{$ppid})
                {
                    $ppid_arr{$ppid} = $pid;
                }
			}

            foreach my $ppid(keys %ppid_arr)
            {
                if(exists $pid_arr{$ppid})
                {
                    next;
                }

				kill 9,$ppid_arr{$ppid};
            }
		}
...
sub start_process
{
	...
	if($port != 0)
	{
		...
		if($result eq "")
		{
			...
			if($name eq "Freesvr_RDP" || $name eq "Freesvr_PLAY" || $name eq "Freesvr_MONITOR")
			{
				sleep 5;
			}
			else
