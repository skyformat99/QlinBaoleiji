...
sub fork_process
{
	...
	if ($pid == 0)
	{
		...
		if($temp->[1] eq 'cpu')
		{
			...
			&insert_table($dbh,$temp->[0],undef,'snmp_month_report_cpu',@results);
		}
		elsif($temp->[1] eq 'memory')
		{
			my @results = &rrd_fetch($temp->[3],'val');
			&insert_table($dbh,$temp->[0],undef,'snmp_month_report_memory',@results);
		}
		elsif($temp->[1] eq 'swap')
		{
			my @results = &rrd_fetch($temp->[3],'val');
			&insert_table($dbh,$temp->[0],undef,'snmp_month_report_swap',@results);
		}
		elsif($temp->[1] eq 'disk')
		{
			my @results = &rrd_fetch($temp->[3],'val');
			&insert_table($dbh,$temp->[0],$temp->[2],'snmp_month_report_disk',@results);
		}
...
sub insert_table
{
	...
	$select_table =~ s/month/day/g;

	my $sqr_select;
	unless(defined $disk)
	{
		$sqr_select = $dbh->prepare("select sum(novalue) from $select_table where device_ip = '$device_ip' and datetime >= now()-interval 1 month and datetime < now()");
	}
	else
	{
		$sqr_select = $dbh->prepare("select sum(novalue) from $select_table where device_ip = '$device_ip' and disk = '$disk' and datetime >= now()-interval 1 month and datetime < now()");
	}
	$sqr_select->execute();
