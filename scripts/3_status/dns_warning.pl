...
sub mail_alarm_process
{
    ...
    my $sqr_select = $dbh->prepare("select id,device_ip,type,datetime,cur_val,context from dns_warning_log where mail_status = -1 order by datetime desc");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        ...
        {
            foreach my $deviceIp(keys %dns_errlog)
            {
                unless(exists $device_errlog{$deviceIp})
                {
                    $device_errlog{$deviceIp} = "$deviceIp dns延时告警:\n";
                }
                $device_errlog{$deviceIp} .= "时间: $last_datetime\n";
                $device_errlog{$deviceIp} .= "$dns_errlog{$deviceIp}";
            }

            foreach my $deviceIp(keys %dns_errlog)
            {
                delete $dns_errlog{$deviceIp};
            }
        }
        ...
        unless(exists $dns_errlog{$device_ip})
        {
            $dns_errlog{$device_ip} = "";
        }

        $dns_errlog{$device_ip} .= "$context\n";
    }
    $sqr_select->finish();
    ...
    {
        my $subject = "主机dns延时告警 $time_now_subject";
    ...
    foreach my $device_ip(keys %device_status)
    {
        ...
        {
            my $sqr_update = $dbh->prepare("update dns_warning_log set mail_status = $status where id = $id");
            $sqr_update->execute();
            $sqr_update->finish();

            if($status == 1)
            {
                $sqr_select = $dbh->prepare("select type from dns_warning_log where id=$id");
                $sqr_select->execute();
                my $ref_select = $sqr_select->fetchrow_hashref();
                my $tmp_type = $ref_select->{"type"};
                $sqr_select->finish();

                $sqr_select = $dbh->prepare("select id from dns_status where device_ip='$device_ip' and type=$tmp_type");
                $sqr_select->execute();
                ...
                if(defined $id)
                {
                    $sqr_update = $dbh->prepare("update dns_status set mail_last_sendtime = '$time_now_str' where id=$id");
                    $sqr_update->execute();
                    $sqr_update->finish();
                }
            }
...
sub sms_alarm_process
{
    ...
    my $sqr_select = $dbh->prepare("select id,device_ip,type,datetime,cur_val,context from dns_warning_log where sms_status = -1 order by datetime desc");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        my $id = $ref_select->{"id"};
    ...
    foreach my $device_ip(keys %device_status)
    {
        ...
        foreach my $id(@{$device_status{$device_ip}->[0]})
        {
            my $sqr_update = $dbh->prepare("update dns_warning_log set sms_status = $status where id = $id");
            $sqr_update->execute();
            $sqr_update->finish();
            ...
            if($status == 1)
            {
                ...
                if(defined $id)
                {
                    $sqr_update = $dbh->prepare("update dns_status set sms_last_sendtime = '$time_now_str' where id=$id");
                    $sqr_update->execute();
                    $sqr_update->finish();
                }
            }
