...
my $sqr_select = $dbh->prepare("select device_ip,port,app_name,app_type,rrdfile from app_status where enable=1 and rrdfile != '' and rrdfile is not null");
$sqr_select->execute();
while(my $ref_select = $sqr_select->fetchrow_hashref())
{
	...
	my @tmp = ($device_ip,$port,$app_name,$app_type,$rrdfile);
	push @ref_ip_arr,\@tmp;
}
$sqr_select->finish();
...
sub fork_process
{
	...
	if ($pid == 0)
	{
        ...
        &insert_table($dbh,$device_ip,$port,$app_name,$app_type,@results);
		exit 0;
	}
	++$exist_process;
}
...
sub insert_table
{
	my($dbh,$device_ip,$port,$app_name,$app_type,$avg,$max,$min,$nan_num) = @_;
    ...
    my $sqr_insert = $dbh->prepare("insert into app_month_report(device_ip,port,date,app_name,app_type,avg,noval) values('$device_ip',$port,'$date_now_str','$app_name','$app_type',$avg,$nan_num)");
	$sqr_insert->execute();
	$sqr_insert->finish();
}
