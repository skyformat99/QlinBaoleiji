...
sub mail_alarm_process
{
    ...
    my $sqr_select = $dbh->prepare("select id,device_ip,datetime,process,val,context from snmp_check_process_warning_log where mail_status=-1 order by datetime desc");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        ...
        my $process = $ref_select->{"process"};
        my $val = $ref_select->{"val"};
        my $context = $ref_select->{"context"};

        if(defined $last_datetime && $last_datetime ne $datetime)
        {
            foreach my $deviceIp(keys %process_log)
            {
                unless(exists $device_errlog{$deviceIp})
                {
                    $device_errlog{$deviceIp} = "$deviceIp snmp进程扫描告警:\n";
                }   
                $device_errlog{$deviceIp} .= "时间: $last_datetime\n";
                $device_errlog{$deviceIp} .= "$process_log{$deviceIp}";

                if(exists $process_cache{$deviceIp} && scalar @{$process_cache{$deviceIp}->[0]} != 0)
                {
                    $device_errlog{$deviceIp} .= join(",",@{$process_cache{$deviceIp}->[0]});
                    $device_errlog{$deviceIp} .= " 无法得到值\n";
                }

                if(exists $process_cache{$deviceIp} && scalar @{$process_cache{$deviceIp}->[1]} != 0)
                {
                    $device_errlog{$deviceIp} .= join(",",@{$process_cache{$deviceIp}->[1]});
                    $device_errlog{$deviceIp} .= " 进程不存在\n";
                }
            }

            foreach my $deviceIp(keys %process_log)
            {
                delete $process_log{$deviceIp};
                delete $process_cache{$deviceIp};
            }

            foreach my $deviceIp(keys %process_cache)
            {
                unless(exists $device_errlog{$deviceIp})
                {
                    $device_errlog{$deviceIp} = "$deviceIp snmp进程扫描告警:\n";
                ...
                if(scalar @{$process_cache{$deviceIp}->[0]} != 0)
                {
                    $device_errlog{$deviceIp} .= join(",",@{$process_cache{$deviceIp}->[0]});
                    $device_errlog{$deviceIp} .= " 无法得到值\n";
                }

                if(scalar @{$process_cache{$deviceIp}->[1]} != 0)
                {
                    $device_errlog{$deviceIp} .= join(",",@{$process_cache{$deviceIp}->[1]});
                    $device_errlog{$deviceIp} .= " 进程不存在\n";
                }
        ...
        if(defined $process)
        {
            if($val<0)
            {
                unless(exists $process_cache{$device_ip})
                {
                    my @tmp1; my @tmp2;
                    my @tmp = (\@tmp1,\@tmp2);
                    $process_cache{$device_ip} = \@tmp;
                }
                push @{$process_cache{$device_ip}->[0]},$process;
            }
            else
            {
                unless(exists $process_cache{$device_ip})
                ...
                push @{$process_cache{$device_ip}->[1]},$process;
            }
        }
        else
        {
            unless(exists $process_log{$device_ip})
            {
                $process_log{$device_ip} = "";
            }
            $process_log{$device_ip} .= "$context\n";
        }
    }
    $sqr_select->finish();
...
#            my $email = 'wxl890206@126.com';
    ...
    {
        my $subject = "主机进程状态告警 $time_now_subject";
    ...
    foreach my $device_ip(keys %device_status)
    {
        ...
        {
            my $sqr_update = $dbh->prepare("update snmp_check_process_warning_log set mail_status = $status where id=$id");
            $sqr_update->execute();
            $sqr_update->finish();

            if($status == 1)
            {
                my $sqr_select_process = $dbh->prepare("select process from snmp_check_process_warning_log where id=$id");
                $sqr_select_process->execute();
                my $ref_select_process = $sqr_select_process->fetchrow_hashref();
                my $tmp_process = $ref_select_process->{"process"};
                $sqr_select_process->finish();

                if(defined $tmp_process)
                {
                    $sqr_update = $dbh->prepare("update snmp_check_process set mail_last_sendtime = '$time_now_str' where device_ip='$device_ip' and process='$tmp_process'");
                    $sqr_update->execute();
                    $sqr_update->finish();
                }
                else
                {
                    $sqr_update = $dbh->prepare("update snmp_check_process set mail_last_sendtime = '$time_now_str' where device_ip='$device_ip'");
                    $sqr_update->execute();
                    $sqr_update->finish();
                }
            }
...
sub sms_alarm_process
{
    ...
    my $sqr_select = $dbh->prepare("select id,device_ip,datetime,process,val,context from snmp_check_process_warning_log where sms_status=-1 order by datetime desc");
    $sqr_select->execute();
    while(my $ref_select = $sqr_select->fetchrow_hashref())
    {
        my $id = $ref_select->{"id"};
    ...
    foreach my $device_ip(keys %device_status)
    {
        ...
        foreach my $id(@{$device_status{$device_ip}->[0]})
        {
            my $sqr_update = $dbh->prepare("update snmp_check_process_warning_log set sms_status = $status where id=$id");
            $sqr_update->execute();
            $sqr_update->finish();
            ...
            if($status == 1)
            {
                ...
                if(defined $tmp_process)
                {
                    $sqr_update = $dbh->prepare("update snmp_check_process set sms_last_sendtime = '$time_now_str' where device_ip='$device_ip' and process='$tmp_process'");
                    $sqr_update->execute();
                    $sqr_update->finish();
                }
                else
                {
                    $sqr_update = $dbh->prepare("update snmp_check_process set sms_last_sendtime = '$time_now_str' where device_ip='$device_ip'");
                    $sqr_update->execute();
                    $sqr_update->finish();
                }
            }
