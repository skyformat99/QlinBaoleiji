...
class c_admin_session extends c_base
{
    function index($where = NULL, $interface=false)
    {
        ...
        if ($delete) {
            ...
            if($interface){
	        	return array('total'=>$row_num, 'datas'=>$allsession);
	        }
    ...
    function search()
    {
        ...
        if ($_SESSION['ADMIN_LEVEL'] != 1) {
            ...
            $this->assign('alldev', $this->server_set->select_all('1', 'device_ip', 'asc'));
        }
    ...
    function download($interface=0)
    {
		...
		if($interface&&$_SESSION['ADMIN_LEVEL']==0){
			if($row=$this->sessions_set->select_by_id($sid)){
				if($row['luser']!=$_SESSION['ADMIN_USERNAME']){
					alert_and_close('您没有权限查看');
					return;
    ...
    function gateway_running_list($r=0, $pre='')
    {
        ...
        if($r==1) {
            return array('total'=>$row_num, 'datas'=>$targets);
        }
    ...
    function search_html_log($interface=false)
    {
        global $_CONFIG;
        $ac = get_request("ac", 1, 1);
		$page_num = get_request('page');
        ...
        if ($contents) {
            ...
            */
			...
			if($ssh_or_rdp == 'rdp'){
				...
				$newpager = new my_pager($row_num, $page_num, $this->config['site']['items_per_page'], 'page');
				$alllog = $this->search_html_set->select_limit($newpager->intStartPosition, $newpager->intItemsPerPage, $where, 'start', 'desc');
				$this->assign('page_list', $newpager->showSerialList());
				$this->assign('session_num', $row_num);
				$this->assign('curr_page', $newpager->intCurrentPageNumber);
				$this->assign('total_page', $newpager->intTotalPageCount);
