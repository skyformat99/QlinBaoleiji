...
class c_admin_app extends c_base {
	...
	function app_group() {
		...
		if($username){
			$where .= ' AND id IN(select appresourceid FROM '.$this->luser_appresourcegrp_set->get_table_name().' WHERE memberid IN (select uid from '.$this->member_set->get_table_name().' where username like "%'.($username).'%" or realname like "%'.($username).'%") union select appresourceid FROM '.$this->lgroup_appresourcegrp_set->get_table_name().' WHERE groupid IN(select groupid from '.$this->member_set->get_table_name().' where username  like "%'.($username).'%" or realname  like "%'.($username).'%"))';
		}elseif($groupid){
			$where .= ' AND id IN(select appresourceid FROM '.$this->lgroup_appresourcegrp_set->get_table_name().' WHERE groupid IN('.$groupid.'))';
		}
		if(!empty($groupname)){
			$where .= " AND appgroupname like '%".$groupname."%'";
		}
	...
	function app_group_edit() {
		...
		if($_POST){
			...
			$res = $this->appdevice_set->base_select("SELECT d.*,a.name apppubname, a.appserverip,a.appprogramname  FROM appdevices d LEFT JOIN apppub a ON d.apppubid=a.id  WHERE a.id IS NOT NULL AND d.id IN(".implode(',', $selectdevice).") ".($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101 ? ' AND appserverip IN(\''.$whereappserver.'\')' : '')."  ORDER BY appserverip");

			$res2 = $this->devpass_set->base_select("SELECT a.groupid,a.username,b.groupname FROM ".$this->appresgroup_set->get_table_name()." a left join ".$this->sgroup_set->get_table_name()." b ON a.groupid=b.id WHERE b.id IN(".implode(',', $selectgroup).") AND a.appgroupname='".$ginfo['appgroupname']."' and appdevicesid=-1 ORDER BY convert(b.groupname using gbk) ASC");
		}else{
