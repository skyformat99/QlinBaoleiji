...
class c_admin_member extends c_base {
	...
	function memberderive($where='1'){
		...
		foreach($result as $info) {
			...
			if(!$_SESSION['RADIUSUSERLIST']){
				...
				$str .= $info['asyncoutpass'].",";
				switch($info['tranportauth']){
					case 1:
						$str .= "本地,";
						break;
					case 2:
	...
	function add() {
		...
		$member['sshport'] = $sshport[0]['svalue'];
		$member['rdpport'] = $rdpport[0]['svalue'];
		...
		$appserverlist = $this->appserver_set->base_select("select a.*,b.hostname from ".$this->appserver_set->get_table_name()." a LEFT JOIN ".$this->server_set->get_table_name()." b ON a.appserverip=b.device_ip order by INET_ATON(a.appserverip) asc");
		if($appserverlist)
		$appserverlist[0]['check']='checked';
		$this->assign('appserverlist',$appserverlist);
...
*/		
		...
		$logintype['ldapauth']=$member['ldapauth'];
		$logintype['adauth']=$member['adauth'];
		$logintype['emailauth']=$member['emailauth'];
		...
		$appserverlist = $this->appserver_set->base_select("select a.*,b.hostname from ".$this->appserver_set->get_table_name()." a LEFT JOIN ".$this->server_set->get_table_name()." b ON a.appserverip=b.device_ip order by INET_ATON(a.appserverip) asc");
		if($member['appserver']){
			for($i=0; $i<count($appserverlist); $i++){
				$appserver_arr = explode(',',$member['appserver']);
				if(in_array($appserverlist[$i]['id'], $appserver_arr)){
					$appserverlist[$i]['check']='checked';
				}
			}
		}
		$this->assign('appserverlist',$appserverlist);
	...
	function save($interface=false) {
		...
		$_SESSION['MEMBERADDTMP']['emailauth']=$emailauth = get_request('emailauth', 1, 0);
		...
		$_SESSION['MEMBERADDTMP']['sshtimeout']=$sshtimeout = get_request('sshtimeout', 1, 0);
		$_SESSION['MEMBERADDTMP']['rdptimeout']=$rdptimeout = get_request('rdptimeout', 1, 0);
		$_SESSION['MEMBERADDTMP']['sftp']=$sftp = get_request('sftp', 1, 0);
		$_SESSION['MEMBERADDTMP']['publicfloder']=$publicfloder = get_request('publicfloder', 1, 1);
		$_SESSION['MEMBERADDTMP']['privatefloder']=$privatefloder = get_request('privatefloder', 1, 1);
		if($_SESSION['RADIUSUSERLIST']){
			$enable='on';
		}
		...
		$newmember->set_data('sshtimeout',$sshtimeout);
		$newmember->set_data('rdptimeout',$rdptimeout);
		$newmember->set_data('sftp',$sftp);
		if($_POST['appservers']){
			$newmember->set_data('appserver',implode(',', $_POST['appservers']));
		}
		if($nolimit){
		...
		if($publicfloder == 'on') {
			$newmember->set_data('publicfloder',1);
		}
		else {
			$newmember->set_data('publicfloder',0);
		}
		if($privatefloder == 'on') {
			$newmember->set_data('privatefloder',1);
		}
		else {
			$newmember->set_data('privatefloder',0);
		}
		...
		$newmember->set_data("emailauth", $emailauth);
		...
		if($newmember->get_errnum() == 0) {
			$whereappserverip = '1';
			$appserverstmp_arr[]='0';
			if($_POST['appservers'][0]){
				$appserverstmp = $this->appserver_set->select_all("id IN(".implode(',', $_POST['appservers']).")");
				for($j=0; $j<count($appserverstmp); $j++){
					$appserverstmp_arr[]=$appserverstmp[$j]['appserverip'];
					$appserverstmp_idarr[]=$appserverstmp[$j]['id'];
				}
			}
			if($appserverstmp_idarr)
			$newmember->set_data('appserver', implode(',', $appserverstmp_idarr));
			$whereappserverip .= " AND appserverip IN('".implode("','", $appserverstmp_arr)."')";
			if($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101){
			...
			if($uid == 0) {
				...
				if($this->member_set->select_all("username = '" . $newmember->get_data('username') . "'") == NULL ) {
					...
					if($uid&&$groupid){
						...
						*/
						...
						$appservers = $this->appmember_set->base_select("SELECT d.device_ip appserverip,a.name hostname,s.groupid FROM ".$this->devpass_set->get_table_name()." d LEFT JOIN ".$this->server_set->get_table_name()." s ON d.device_ip=s.device_ip left join apppserver a ON s.device_ip=a.appserverip  WHERE d.login_method=26  and a.id is not null and $whereappserverip");
				...
				$user = $this->member_set->select_by_id($uid);
				if(empty($emptypassword)&&$user['password'] != $this->member_set->udf_encrypt($newmember->get_data("password"))){
				...
				$appdevices_id = $this->devpass_set->select_all("login_method=26");
				$_appdevices_id[]=0;
				for($a_i=0; $a_i<count($appdevices_id); $a_i++){
					$_appdevices_id[]=$appdevices_id[$a_i]['id'];
				}
				/*$sql = "SELECT devicesid FROM ".$this->luser_set->get_table_name()." WHERE devicesid IN(".implode(',', $_appdevices_id).") AND memberid=".$user['uid'];
				$sql .= " UNION SELECT devicesid FROM ".$this->lgroup_set->get_table_name()." WHERE devicesid IN(".implode(',', $_appdevices_id).") AND groupid=".$user['groupid'];
				$sql .= " UNION  SELECT devicesid FROM ".$this->resgroup_set->get_table_name()." WHERE devicesid IN(".implode(',', $_appdevices_id).") AND groupname IN (SELECT b.groupname FROM ".$this->luser_resourcegrp_set->get_table_name()." a LEFT JOIN ".$this->resgroup_set->get_table_name()." b ON a.resourceid=b.id WHERE  a.memberid=".$user['uid']." )";
				$sql .= " union select distinct devices.id devicesid from ".$this->luser_resourcegrp_set->get_table_name()." a left join (select a.id,b.groupid,b.username from ".$this->resgroup_set->get_table_name()." a left join ".$this->resgroup_set->get_table_name()." b on a.groupname=b.groupname where a.devicesid=0 and b.devicesid=-1 ) t on a.resourceid=t.id left join servers s on s.groupid=t.groupid left join devices on devices.device_ip=s.device_ip where devices.id IN(".implode(',', $_appdevices_id).") AND t.id and devices.id  and t.groupid and t.groupid=s.groupid AND IF(t.username='0', 1, t.username=devices.username) and  a.memberid=".$user['uid']." ";
				$sql .= " UNION SELECT devicesid FROM ".$this->resgroup_set->get_table_name()." WHERE groupname IN (SELECT b.groupname FROM ".$this->lgroup_resourcegrp_set->get_table_name()." a LEFT JOIN ".$this->resgroup_set->get_table_name()." b ON a.resourceid=b.id WHERE devicesid IN(".implode(',', $_appdevices_id).") AND a.groupid=".$user['groupid']." )";	
				$sql .= " UNION select distinct devices.id devicesid from ".$this->lgroup_resourcegrp_set->get_table_name()." a left join (select a.id,b.groupid,b.username from ".$this->resgroup_set->get_table_name()." a left join ".$this->resgroup_set->get_table_name()." b on a.groupname=b.groupname where a.devicesid=0 and b.devicesid=-1 ) t on a.resourceid=t.id left join servers s on s.groupid=t.groupid left join devices on devices.device_ip=s.device_ip where devices.id IN(".implode(',', $_appdevices_id).") AND t.id and devices.id  and t.groupid and t.groupid=s.groupid AND IF(t.username='0', 1, t.username=devices.username) and a.groupid=".$user['groupid']." ";

				$appservers = $this->appmember_set->base_select("SELECT d.device_ip appserverip,a.name hostname,s.groupid FROM ".$this->devpass_set->get_table_name()." d LEFT JOIN ($sql) dd ON d.id=dd.devicesid LEFT JOIN ".$this->server_set->get_table_name()." s ON d.device_ip=s.device_ip left join apppserver a ON s.device_ip=a.appserverip  WHERE dd.devicesid IS NOT NULL AND d.login_method=26  and a.id is not null and $whereappserverip");
				*/
				$appservers = $this->appmember_set->base_select("SELECT d.device_ip appserverip,a.name hostname,s.groupid FROM ".$this->devpass_set->get_table_name()." d LEFT JOIN ".$this->server_set->get_table_name()." s ON d.device_ip=s.device_ip left join apppserver a ON s.device_ip=a.appserverip  WHERE d.login_method=26  and a.id is not null and $whereappserverip");
				if($_CONFIG['Async']&&!$_SESSION['RADIUSUSERLIST']){
					for($i=0; $i<count($appservers); $i++){
	...
	function save_self() {
		...
		$toolhostname = get_request('toolhostname', 1, 0);
		...
		$newmember->set_data('sshport', $sshport);
		$newmember->set_data('rdpport', $rdpport);
		$newmember->set_data('toolhostname', $toolhostname);

		if($email&&!is_email($email)){
			alert_and_back('Email格式不正确');
			exit();
		}
		if($user['level']!=11&&$rows<=0){
		...
		if(!($password1 == "" && $password2 == "")) {
			...
			if($password1 == $password2) {
				...
				$whereappserverip = '1';
				$appserverstmp_arr[]='0';
				$appserverstmp_idarr = array();
				if($user['appserver']){
					$appserverstmp = $this->appserver_set->select_all("id IN(".$user['appserver'].")");
					for($j=0; $j<count($appserverstmp); $j++){
						$appserverstmp_arr[]=$appserverstmp[$j]['appserverip'];
						$appserverstmp_idarr[]=$appserverstmp[$j]['id'];
					}
		...
		if($newmember->get_errnum() == 0) {
			...
			if($_CONFIG['Async']&&$user['level']!=11){
				for($i=0; $i<count($appservers); $i++){
					echo $cmd = "async ".$appservers[$i]['appserverip']." 8888 ".$_SESSION['ADMIN_USERNAME']." 1";
					//echo '<br />';
					$r=asyncAccount($cmd);
				}
			}
	...
	function domemberimport(){
		...
		for($i=1; $i<count($lines); $i++){
			...
			if(!$_SESSION['RADIUSUSERLIST']){
				...
				$asyncoutpass =$linearr[$index_i++];
				$tranportauth =$linearr[$index_i++];
				switch(strtolower($tranportauth)){
					case '本地':
						$tranportauth=1;
						break;
					case 'radius':
