...
class c_admin_forbidden extends c_base {
	...
	function forbiddengps_edit(){
		...
		$member = $this->member_set->select_all("level=1 or level=3 or level=2 or level=21");
	...
	function forbiddengps_save($interface=false){
		...
		$gname = get_request('gname', 1,1);
		$adminuser = get_request('adminuser', 1,0);
		...
		if($add == 'new'){
			...
			$forbiddengps->set_data('adminuser', $adminuser);
	...
	function del_forbiddengps($interface=false) {
		$gid = $_GET['gid'];
		if(is_array($gid)){
			for($i=0; $i<count($gid); $i++){
				$ginfo = $this->forbiddengps_set->select_by_id($gid[$i]);
				if($interface){
					$this->luser_set->query("UPDATE luser SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid[$i].")");
					$this->luser_set->query("UPDATE lgroup SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid[$i].")");
					$this->luser_set->query("UPDATE luser_resourcegrp SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid[$i].")");
					$this->luser_set->query("UPDATE lgroup_resourcegrp SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid[$i].")");
				}
				else
			...
			if($interface){
				$this->luser_set->query("UPDATE luser SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid.")");
				$this->luser_set->query("UPDATE lgroup SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid.")");
				$this->luser_set->query("UPDATE luser_resourcegrp SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid.")");
				$this->luser_set->query("UPDATE lgroup_resourcegrp SET forbidden_commands_groups='' WHERE forbidden_commands_groups=(select gname from forbidden_groups where gid=".$gid.")");
			}
			else
