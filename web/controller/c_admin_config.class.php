...
class c_admin_config extends c_base {
	...
	function login_save(){
		...
		if(!ctype_digit($oldpassnumber)){
			alert_and_back('记忆旧密码次数请输入整数');
			exit;
		}
	...
	function appserver_list($interface=false) {
		global $_CONFIG;
		$page_num = get_request('page');
		$orderby1 = get_request('orderby1', 0, 1);
		$orderby2 = get_request('orderby2', 0, 1);
	...
	function appserver_save() {
		...
		$outsideip = get_request("outsideip", 1, 1);
		...
		if($this->appserver_set->select_count("appserverip = '$appserverip' AND id!='$id'")) {
			...
			$newappserver->set_data('outsideip',$outsideip);
			...
			for($ii=0; $ii<count($lines); $ii++)
			{
				if(empty($endline)){
					...
					if(empty($endline)&&strstr(strtolower($lines[$ii]), "commit")){
						$endline = $ii;
					}
				}
			}
			...
			if(empty($foundip)){
				if($endapp){
					...
					$lines[$endline]="#app config start\n";
					$lines[$endline+1]="-A  RH-Firewall-1-INPUT -s  $appserverip  -j ACCEPT\n";
					$lines[$endline+2]="#app config end\n";
				}
			}
	...
	function apppub_list($interface=false) {
		global $_CONFIG;
		$page_num = get_request('page');
		...
		$this->apppub_set->query("delete from appdevices where apppubid not in(select id from apppub)");
		$this->apppub_set->query("delete from apppub where id not in(select apppubid from appdevices)");
	...
	function apppubexport(){
		...
		$result = $this->apppub_set->base_select("SELECT a.username,a.device_ip,c.groupid,a.cur_password password,b.name,b.path,b.appprogramname,b.url,b.description FROM ".$this->appdevice_set->get_table_name()." a LEFT JOIN ".$this->apppub_set->get_table_name()." b ON a.apppubid=b.id LEFT JOIN ".$this->server_set->get_table_name()." c ON a.device_ip=c.device_ip WHERE $where");
		...
		for($i=0; $i<count($result); $i++){
			...
			$result[$i]['password']=stripslashes($this->server_set->udf_encrypt($this->server_set->udf_decrypt($result[$i]['password']),1));
	...
	function doapppubimport(){
		...
		for($i=1; $i<count($lines); $i++){
			...
			if(empty($groupid)){
				$error[]='应用名称"'.$linearr[0].'":'.'组"'.$groupname.'"不存在';
				continue;
			}
			...
			if($info=$this->apppub_set->select_all("name='".$linearr[0]."' AND appserverip='$appserverip'" )){
				...
				for($info_i=0; $info_i<count($info); $info_i++){
					...
					$linearr[2]=($encrypt ? $this->apppub_set->udf_encrypt($linearr[2]) : $this->apppub_set->udf_encrypt($this->apppub_set->udf_decrypt(addslashes($linearr[2]),1)));
	...
	function appprogram_list($interface=false) {
		exec("sudo /opt/freesvr/audit/sbin/license-print", $out, $return);
		$_o = explode(' ', $out[0]);
		$_p = explode('-', $_o[3]);
		if(empty($_p[4])){
		...
		if($interface) return $appprogram;
		$this->assign('page_list', $newpager->showSerialList());
		$this->assign('command_num', $row_num);
		$this->assign('curr_page', $newpager->intCurrentPageNumber);
		$this->assign('total_page', $newpager->intTotalPageCount);
			...
			if($appprogram->get_errnum() == 0) {
				...
				if(empty($id)){
					...
					if($old['autologin']!=$autologin){
						$this->appprogram_set->query("UPDATE apppub set autologinflag='".$autologin."' where autologinflag='".$old['autologin']."' and appprogramname='".$name."'");
					}
	...
	function appicon_list($interface=false) {
		exec("sudo /opt/freesvr/audit/sbin/license-print", $out, $return);
		$_o = explode(' ', $out[0]);
		$_p = explode('-', $_o[3]);
		if(empty($_p[4])){
	...
	function appicon_save($interface=false) {//var_dump($_POST);
		$id = get_request("id", 0, 1);
		$name = get_request("name", 1, 1);
		$path = get_request("path", 1, 1);
		if(empty($name)) {
	...
	function appdevice_edit( $serverip, $apppubid, $appdeviceid) {
		...
		$this->assign("allmem", $allmem);
		$this->assign("usergroup", $usergroup);		
		$this->assign("device_ip", is_ip($device_ip) ? $device_ip : '');
		$this->assign('sessionluser', $sessionluser);
		$this->assign('sessionlgroup', $sessionlgroup);
	...
	function appdevice_save($apppubid, $apppubdevices) {
		...
		$serverid = get_request('serverid', 1, 0);
		...
		$oracle_db = get_request("oracle_db", 1, 1);
		$navicat_port = get_request("navicat_port", 1, 1);
		$sqlserver_port = get_request("sqlserver_port", 1, 1);
		$sqlserver_port1 = get_request("sqlserver_port1", 1, 1);
		$ishostname = get_request("ishostname", 1, 0);
		...
		if($oracle_name&&$this->appdevice_set->select_count("oracle_name='".$oracle_name."' AND apppubid IN(select id from apppub where appserverip='".$appserverip."') AND apppubid!=$id")>0){
			//alert_and_back("实例名称已存在","admin.php?controller=admin_config&action=apppub_edit&id=$apppubid&appserverip=$appserverip&device_ip=".get_request('device_ip', 0, 1));
			//return;	
		}
		if($serverid){
			$serverinfo = $this->server_set->select_by_id($serverid);
			$device_ip = $serverinfo['device_ip'];
			if(empty($serverinfo)){
				alert_and_back("选择servers有误");
				return;	
			}
			if($ishostname){
				$device_ip = $serverinfo['hostname'];
			}
		}
		if($password!=$repassword){
			...
			if($password){
				if($id){
					...
					$newappdeviceid->set_data('old_password',$this->member_set->udf_encrypt($this->member_set->udf_decrypt($old['cur_password'])));
				}
			...
			$newappdeviceid->set_data('ishostname', $ishostname);
			$newappdeviceid->set_data('oracle_name', $oracle_name);
			$newappdeviceid->set_data('oracle_db', $oracle_db);
			$newappdeviceid->set_data('serverid', $serverid);
			$newappdeviceid->set_data('navicat_port', $navicat_port);
			$newappdeviceid->set_data('sqlserver_port', $sqlserver_port);
			if($sqlserver_port1)
			$newappdeviceid->set_data('sqlserver_port', $sqlserver_port1);
			...
			if($newappdeviceid->get_errnum() == 0) {
				if(empty($id)){
					...
					for($i=0; $i<count($_POST['member']); $i++){		
						if(empty($_POST['member'][$i])) continue;
						...
						/*$__member = $this->member_set->select_by_id($_POST['member'][$i]);
						$cmd = "async ".$appserverip." 8888 ".$__member['username']." 1";
						$rs = asyncAccount($cmd);
						if($rs<0||intval(substr($rs, 0, 1))==0){
							$asyncaccount_errorip[] = $__member['username'];
						}*/
					}
					for($i=0; $i<count($_POST['group']); $i++){
						if(empty($_POST['group'][$i])) continue;
						...
						$where = "devices.id=".$apppubdevices[0]['id']." AND a.groupid=".$_POST['group'][$i];
						...
						/*$_members = $this->member_set->select_all("groupid=".$_POST['group'][$i]);
						for($m_i=0; $m_i<count($_members); $m_i++){
							$cmd = "async ".$appserverip." 8888 ".$_members[$m_i]['username']." 1";
							$rs = asyncAccount($cmd);
							if($rs<0||intval(substr($rs, 0, 1))==0){
								$asyncaccount_errorip[] = $_members[$m_i]['username'];
							}
						}*/
					}
					unset($_SESSION[$sessionluser]);
					unset($_SESSION[$sessionlgroup]);
					alert_and_back("添加成功".($asyncaccount_errorip ? ',同步账号:'.implode(' ', $asyncaccount_errorip)." 到应用发布服务器($appserverip)失败 " : ''), "admin.php?controller=admin_config&action=apppub_list&ip=$appserverip&device_ip=".get_request('device_ip', 0, 1));	
					return;	
				}else{
					...
					unset($_SESSION[$sessionluser]);
					unset($_SESSION[$sessionlgroup]);
					alert_and_back("修改成功".($asyncaccount_errorip ? ',同步账号:'.implode(' ', $asyncaccount_errorip)." 到应用发布服务器($appserverip)失败 " : ''), "admin.php?controller=admin_config&action=apppub_list&ip=$appserverip&device_ip=".get_request('device_ip', 0, 1));	
					return;
				}
			} else {
				alert_and_back($newapppub->get_firsterr(), NULL, 1);
	...
	function ha()
	{
		...
		global $_CONFIG;
		exec('sudo /opt/freesvr/audit/bin/mysql_ha_scan.py');
		$ha = $this->setting_set->base_select("SELECT * FROM hastatus LIMIT 1");
		for($i=0; $i<$_CONFIG['CONFIGFILE']['IFGETH_NUMBER']; $i++){
			if(file_exists($_CONFIG['CONFIGFILE']['IFGETH'.$i])){
				$filename=substr($_CONFIG['CONFIGFILE']['IFGETH'.$i], strrpos($_CONFIG['CONFIGFILE']['IFGETH'.$i],'/')+1);
				$eth[]=array('i'=>$i,'name'=>strtoupper(substr($filename,strpos($filename,'-')+1)));
			}
		}
		$lines = @file('/etc/hosts');
		for($i=0; $i<count($lines); $i++){
			$tmp = preg_split("/[\s]+/", $lines[$i]);
			if(trim($tmp[1])=='dbaudithost'){
				$ha[0]['dbaudithost']=trim($tmp[0]);
				break;
			}
		}
		$this->assign("ha", $ha[0]);
	...
	function ha_save()
	{
		//$filename = './controller/global.cfg';
		global $_CONFIG;		
		$masterip = get_request('masterip',1,1);	
		$password = get_request('password',1,1);
		$slaveip = get_request('slaveip',1,1);
		$dbaudithost = get_request('dbaudithost',1,1);
		$eth = get_request('eth',1,0);
		...
		if($dbaudithost){
			...
			for($i=0; $i<count($lines); $i++){
				...
				if(trim($tmp[1])=='dbaudithost'){
					if(!$found){
						$_lines[]=$dbaudithost." dbaudithost\n";
						$found = true;
					}
				}else{
					$_lines[]=$lines[$i];
			...
			$this->Array2File($_lines,'/etc/hosts');
			unset($lines);
		...
		if(!is_ip($masterip)){
			alert_and_back('请输入正确的对端IP');
			exit;
		}
		if(!($password)){
			alert_and_back('root密码');
			exit;
		}
		if(!is_numeric($eth)){
			alert_and_back('请选择eth接口');
			exit;
		}

		if($slaveip&&!is_ip($slaveip)){
			alert_and_back('请输入正确的浮动IP');
			exit;
		}
		
		unset($_lines);
		$linestmp = file($_CONFIG['CONFIGFILE']['IFGETH'.$eth]);
		for($ii=0; $ii<count($linestmp); $ii++)
		{
			...
			if(strstr(strtoupper($linestmp[$ii]), "DEVICE"))
			{
				$tmp = explode("=", $linestmp[$ii]);
				$eth0 = $tmp[1];
			}
		}
		$ipaddr = trim(preg_replace('/\r|\n/', '', $ipaddr)); 
		$eth0 = trim(preg_replace('/\r|\n/', '', $eth0)); 
		exec('sudo /bin/chown  freesvr /home/wuxiaolong/mysql_keepalived/master_php.pl ');
		$linestmp = @file('/home/wuxiaolong/mysql_keepalived/master_php.pl');
		//var_dump($linestmp);
		for($ii=0; $ii<count($linestmp); $ii++)
		{
			if(substr($linestmp[$ii],0,1)=='#') continue;
			//echo $linestmp[$ii].':'.strlen($linestmp[$ii])."<br />";
			if(strstr($linestmp[$ii], "our \$passwd"))
			{
				$linestmp[$ii] = "our \$passwd=\"".$password."\";\n";
			}
			if(strstr($linestmp[$ii], "our(\$peer_ip,\$local_ip,\$interface,\$virtual_ip)"))
			{
				if($slaveip){
					$linestmp[$ii] = "our(\$peer_ip,\$local_ip,\$interface,\$virtual_ip) = (\"".$masterip."\", \"".$ipaddr."\", \"".$eth0."\", \"".$slaveip."\");\n";
				}else{
					$linestmp[$ii] = "our(\$peer_ip,\$local_ip,\$interface,\$virtual_ip) = (\"".$masterip."\", \"".$ipaddr."\", undef, undef);\n";
				}
			}
		}
		$this->Array2File($linestmp,'/home/wuxiaolong/mysql_keepalived/master_php.pl');

		exec('sudo /bin/chown  root /home/wuxiaolong/mysql_keepalived/master_php.pl ');

		$cmd = "sudo /home/wuxiaolong/mysql_keepalived/master_php.pl >/tmp/null&";
		exec($cmd);
	...
	function default_policy(){
		...
		if($ac){
			...
			$this->config_set->query("UPDATE setting set svalue='".$sshport."' WHERE sname='SSHPORT'");
			$this->config_set->query("UPDATE setting set svalue='".$rdpport."' WHERE sname='RDPPORT'");
			alert_and_back('修改成功');
		}
		$sshport = $this->config_set->base_select("SELECT * FROM setting WHERE sname='SSHPORT' LIMIT 1");
		$rdpport = $this->config_set->base_select("SELECT * FROM setting WHERE sname='RDPPORT' LIMIT 1");
		...
		$this->assign("sshport", $sshport[0]['svalue']);
		$this->assign("rdpport", $rdpport[0]['svalue']);
	...
	function config_ftp()
	{
		...
		if(!empty($lines))
		{
			...
			for($ii=0; $ii<count($lines); $ii++)
			{
				...
				if(strstr($lines[$ii], "ssl_protocols"))
				{
					$lines[$ii]=trim($lines[$ii]);
					$lines[$ii]=rtrim($lines[$ii],";");
					$tmp = preg_split("/[\s]+/", strtolower($lines[$ii]));
					$nginxonly12 = in_array('sslv3',$tmp)||in_array('tlsv1',$tmp)||in_array('tlsv1.1',$tmp) ? 0 : (in_array('tlsv1.2',$tmp) ? 1 : 0);
				}
			}
		}
		...
		$fingersecserver = $this->setting_set->select_all("sname='FingerSecServer'");
		$fingersecserver = $fingersecserver[0]['svalue'];
		$network['year'] = date("Y");
		...
		$this->assign("nginxonly12", $nginxonly12);
	...
	function snmpcommunity_save(){
		...
		unset($lines);
		ob_start();
		system('sudo /usr/bin/systemctl  stop snmpd');
		system('sudo /usr/bin/systemctl  start snmpd');
		ob_get_clean();
		alert_and_back('配置成功','admin.php?controller=admin_config&action=config_ftp');
	} 
	...
	function certificate_save(){
		...
		if($_CONFIG['WEBSERVER']){
			...
			if(!empty($lines))
			{
				for($ii=0; $ii<count($lines); $ii++)
				{
					if(strstr($lines[$ii], "ssl_client_certificate"))
					{
						$lines[$ii] = (empty($Certificate) ? '#        ssl_client_certificate /opt/freesvr/web/CA/ca.crt;' : '         ssl_client_certificate /opt/freesvr/web/CA/ca.crt;')."\n";
					}
					if(strstr($lines[$ii], "ssl_verify_client on"))
					{
						$lines[$ii] = (empty($Certificate) ? '#        ssl_verify_client on;' : '         ssl_verify_client on;')."\n";
					}
				}
			}
			else
	...
	function certificatenginvonly12_save(){
		global $_CONFIG;
		$nginxonly12 = get_request('nginxonly12', 1, 0);
		$filename = $_CONFIG['CONFIGFILE']['NGINX'];
		...
		if(!empty($lines))
		{
			for($ii=0; $ii<count($lines); $ii++)
			{
				if(strstr($lines[$ii], "ssl_protocols"))
				{
					$lines[$ii] = (!empty($nginxonly12) ? '         ssl_protocols TLSv1.2;' :'         ssl_protocols SSLv3 TLSv1 TLSv1.1 TLSv1.2;')."\n";
				}
			}
		}
		else
	...
	function settime(){
		...
		$adminlog->set_data('result', language('已记录'));
		$this->admin_log_set->add($adminlog);				
//echo "sudo /usr/bin/date -s '$year$month$day $hour:$minute:$second'";
		exec("sudo /usr/bin/date -s '$year$month$day $hour:$minute:$second'",$out);
		alert_and_back('配置成功','admin.php?controller=admin_config&action=config_ftp');
	}
	...
	function ldapusers(){
		...
		if ($ldapconn) {
			...
			for($i=0; $i<$r['count']; $i++){
				...
				//echo '<pre>';var_dump($r[$i]['samaccountname'][0]);echo '</pre>';
				//var_dump($distinguishedname);var_dump($basedn);var_dump(strtolower($distinguishedname)==strtolower($basedn));
			...
			//echo '<pre>';var_dump($ous);echo '</pre>';
			$groups = array();
