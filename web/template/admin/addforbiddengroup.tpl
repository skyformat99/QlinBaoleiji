...
{{if $smarty.session.ADMIN_LEVEL ne 3 and $smarty.session.ADMIN_LEVEL ne 21 and $smarty.session.ADMIN_LEVEL ne 101}}
  ...
  <tr>
	...
	<tr bgcolor="f7f7f7">
		...
		<td width="67%">
		<select class="wbk"  name="adminuser">
		<option value="0" >无</option>
		{{section name=m loop=$member}}
		<option value="{{$member[m].uid}}" {{if $forbiddengps.adminuser eq $member[m].uid }}selected{{/if}}>{{$member[m].username}}{{$member[m].realname}}</option>
		{{/section}}
		</select>
	  </td>
	</tr>
