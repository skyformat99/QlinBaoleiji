...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_content">
  <tr>
   <td >	
	<form name="f1" method=post action="admin.php?controller=admin_pro&action=resource_group_edit&id={{$ginfo.id}}"  enctype="multipart/form-data" >
...
<input type="hidden" name="oldgname" value="{{$ginfo.groupname}}">
	  <tr>
  <td align="right" colspan="1">系统用户组:<input type=text size="50" name="gname" id="gname" value="{{$ginfo.groupname}}"> </td><td align="left" colspan="2">描述:<input type=text size="50" name="desc" id="desc" value="{{$ginfo.desc}}">
...
<table >
  <tr>
	  ...
	  <tr><td align="right" style="border-bottom:0px;border-top:0px;border-left:0px;border-right:0px;">全选<input type="checkbox" value=2 onclick="checkAll(this.checked);"></td></tr>
		...
		<td width="180"><input type="checkbox" id="uid_{{$allmem[g].uid}}" name='Check{{$smarty.section.g.index}}' value='{{$allmem[g].uid}}'  {{$allmem[g].check}}><a onclick="window.open ('admin.php?controller=admin_pro&action=resourcegrp_seluser&uid={{$allmem[g].uid}}&sid={{$id}}&sessionluser={{$sessionluser}}', 'newwindow', 'height=600, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');return false;" href="#" target="_blank" >{{$allmem[g].username}}({{if $allmem[g].realname}}{{$allmem[g].realname}}{{else}}未设置{{/if}})</a></td>{{if ($smarty.section.g.index +1) % 5 == 0}}</tr><tr>{{/if}}
		{{/if}}
		{{/section}}
		</tr></table>
	  </td>
