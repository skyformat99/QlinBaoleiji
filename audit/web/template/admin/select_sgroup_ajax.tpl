...
departmanagersgroupids = '{{$departmanagersgroupids}}';
		...
		{{$select_group_id}}d.add(0,-1,0,1, '资源组','javascript:setSrcValue(\'\',0,\'{{$select_group_id}}d\',{{if !$inputtype or $inputtype eq "text"}}1{{else}}0{{/if}});hiddenDTree(\'{{$select_group_id}}\');');
