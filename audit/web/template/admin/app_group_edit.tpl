...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_content">
  <tr>
   <td >	<form name="f1" method=post onsubmit="return changegroup();" action="admin.php?controller=admin_app&action=app_group_edit&id={{$ginfo.id}}"  enctype="multipart/form-data" >
...
/*
	...
	if(changed){
		if(confirm('确定要放弃更改?')){
			window.location='admin.php?controller=admin_app&action=app_group_edit&groupid='+gid+'&gname='+gname+'&desc='+desc+'&id={{$ginfo.id}}&ip='+ip+'&hostname='+hostname+'&username='+username+'&appserverip='+appserverip;
		}
	}else{
		window.location='admin.php?controller=admin_app&action=app_group_edit&groupid='+gid+'&gname='+gname+'&desc='+desc+'&id={{$ginfo.id}}&ip='+ip+'&hostname='+hostname+'&username='+username+'&appserverip='+appserverip;
	}
