...
<ul>
	...
	<li class="me_b"><img src="{{$template_root}}/images/an11.jpg" align="absmiddle"/><a href="admin.php?controller=admin_config&action=certs">证书配置</a><img src="{{$template_root}}/images/an33.jpg" align="absmiddle"/></li>
...
<form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_config&action=ftp_save">
	...
	<tr bgcolor=""  onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'');"><td>登录方式:</td>
		<td align=left>
		Radius:<input type="checkbox" class="wbk" name="radiusauth" {{if $logintype.radiusauth}}checked{{/if}} value="1" /> &nbsp;&nbsp;&nbsp;LDAP:<input type="checkbox" class="wbk" name="ldapauth" {{if $logintype.ldapauth}}checked{{/if}} value="1" /> &nbsp;&nbsp;&nbsp;AD:<input type="checkbox" class="wbk" name="adauth" {{if $logintype.adauth}}checked{{/if}} value="1" />&nbsp;&nbsp;&nbsp;指纹认证:<input type="checkbox" class="wbk" name="fingersecauth" {{if $logintype.fingersecauth}}checked{{/if}} value="1" />&nbsp;&nbsp;&nbsp;本地+指纹认证:<input type="checkbox" class="wbk" name="localfingersecauth" {{if $logintype.localfingersecauth}}checked{{/if}} value="1" /> </td>
		<td><input type="submit" class="an_02" value="保存修改"></td>
	...
	<form name="f1" method=post OnSubmit='return check()' action="admin.php?controller=admin_config&action=certificatenginvonly12_save">
	<tr bgcolor="f7f7f7" onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'f7f7f7');"><td>只打开TLS V1.2协议:</td>
		<td align=left>
		<select name="nginxonly12">
		<option value="0" {{if !$nginxonly12}}selected{{/if}}>否</option>
		<option value="1" {{if $nginxonly12}}selected{{/if}}>是</option>
		</select>
		 </td>
		<td><input type="submit" class="an_02" value="保存修改"></td>
...
用户1：<input type=text name="username1" id="username1" size=10 value="" placeholder="过滤用户" onchange="filter(1);" >&nbsp;&nbsp;
			<select  class="wbk" style="width:100px;" id="memberselect1" name="memberselect1"  onchange="usernameselect();">
				...
				{{section name=k loop=$allmem}}
					<option value="{{$allmem[k].uid}}" {{if $allmem[k].uid == $member1}}selected{{/if}}>{{$allmem[k].username}}</option>
				{{/section}}
			</SELECT> &nbsp;&nbsp;
用户2：<input type=text name="username2" id="username2" size=10 value="" placeholder="过滤用户"  onchange="filter(2);" >&nbsp;&nbsp;
			<select  class="wbk" style="width:100px;" id="memberselect2" name="memberselect2"  onchange="usernameselect();">
				<OPTION value="">请选择</OPTION>
				{{section name=k loop=$allmem}}
					<option value="{{$allmem[k].uid}}" {{if $allmem[k].uid == $member2}}selected{{/if}}>{{$allmem[k].username}}</option>
				{{/section}}
			</SELECT> &nbsp;&nbsp;
用户3：<input type=text name="username3" id="username3" size=10 value="" placeholder="过滤用户"  onchange="filter(3);" >&nbsp;&nbsp;
			<select  class="wbk" style="width:100px;" id="memberselect3" name="memberselect3"  onchange="usernameselect();">
				<OPTION value="">请选择</OPTION>
				{{section name=k loop=$allmem}}
					<option value="{{$allmem[k].uid}}" {{if $allmem[k].uid == $member3}}selected{{/if}}>{{$allmem[k].username}}</option>
				{{/section}}
			</SELECT> &nbsp;&nbsp;<br />
用户4：<input type=text name="username4" id="username4" size=10 value="" placeholder="过滤用户"  onchange="filter(4);" >&nbsp;&nbsp;
			<select  class="wbk" style="width:100px;" id="memberselect4" name="memberselect4"  onchange="usernameselect();">
				<OPTION value="">请选择</OPTION>
				{{section name=k loop=$allmem}}
					<option value="{{$allmem[k].uid}}" {{if $allmem[k].uid == $member4}}selected{{/if}}>{{$allmem[k].username}}</option>
				{{/section}}
			</SELECT> &nbsp;&nbsp;
用户5：<input type=text name="username5" id="username5" size=10 value="" placeholder="过滤用户"  onchange="filter(5);" >&nbsp;&nbsp;
			<select  class="wbk" style="width:100px;" id="memberselect5" name="memberselect5"  onchange="usernameselect();">
				<OPTION value="">请选择</OPTION>
				{{section name=k loop=$allmem}}
					<option value="{{$allmem[k].uid}}" {{if $allmem[k].uid == $member5}}selected{{/if}}>{{$allmem[k].username}}</option>
				{{/section}}
			</SELECT> &nbsp;&nbsp;
	...
	<tr bgcolor="f7f7f7"  onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'f7f7f7');"><td>RDP回放IP:</td>
		<td align=left>
		<input type="text" class="" name="rdpplayip" value="{{$rdpplayip}}" /></td>
		<td><input type="submit" class="an_02" value="保存修改"></td>
	...
	<tr bgcolor=""><td>指纹认证服务器wsdl地址:</td>
		<td align=left>
		<input type="text" class="wbk" size="100" name="fingersecserver" value="{{$fingersecserver}}" />
		 </td>
		<td><input type="submit" class="an_02" value="保存修改"></td>
