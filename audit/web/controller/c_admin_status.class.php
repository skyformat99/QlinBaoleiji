...
class c_admin_status extends c_base {
	...
	function latest(){
		...
		$cmd = "sudo /usr/bin/ps -ef | grep keepalived | grep -v grep";
		exec($cmd, $out);
		//var_dump($out);
		if(strstr(strtolower($out[0]), "keepalived"))
		{
			$cmd = "/usr/sbin/ip add show | grep ".$network['virtual_ipaddress'];
			exec($cmd, $out1);
			if(empty($out1)){
				$ha='从';
			}else{
 	...
 	//	print_r($latest);
		...
		$s = $session->gateway_running_list(1,'ssh');
		$this->assign('ssh', $s['datas']);
		$querystring['item']=2;
		arsort($querystring,SORT_STRING);
		$_SERVER['QUERY_STRING'] = 'controller=admin_status&action=latest&item=2&page='.$_GET['page'];//http_build_query($querystring);
		$s = $session->gateway_running_telnet(1,'telnet');
		$this->assign('telnets', $s['datas']);
		...
		$s = $session->index(null,1,'rdprun');
		$this->assign('rdprun', $s['datas']);
		...
		$s = $session->index(null,1,'vncrun');
		$this->assign('vncrun', $s['datas']);
		...
		$s = $session->index(null,1,'apppubrun');
		$this->assign('apppubrun', $s['datas']);
