...
class c_admin_rdp extends c_base {
	function index($where = NULL, $interface=false) {
		...
		if(!is_array($addr)&&is_ip($addr)){
            $where .= " AND addr = '$addr'";
        }else{
        	if(is_array($addr)){
                $where .= " AND(0";
                for($i=0; $i<count($addr); $i++){
                    $where .= " or addr like '%".$addr[$i]."%'";
                }
                $where .= ") ";
            }else{
		...
		if($realname) {
			$where .= " AND luser =(select username from ".$this->member_set->get_table_name()." where realname='$realname')";
		}
		...
		if($delete) {
			...
			//echo $select.$sql." ORDER BY $orderby1 $orderby2 LIMIT $newpager->intStartPosition, $newpager->intItemsPerPage";
			if($interface) {
				return array('total'=>$row_num, 'datas'=>$allsessions);
			}
	...
	function inputview($interface=false){
		...
		if($interface){
			return file_get_contents($sessions['keydir']);
		}
		if(empty($logfile)){
	...
	function downloadplayfile(){
		...
		$filesize = filesize($sessions['replayfile']);  
		$sessions['replayfile']=str_replace("\"", "", $sessions['replayfile']);
		$handle = fopen($sessions['replayfile'],"rb");
	...
	function downloadfile($interface=false){
		$sid = get_request('sid');
		if($_SESSION['ADMIN_LEVEL']==0){
			if($row=$this->rdp_set->select_by_id($sid)){
				if($row['luser']!=$_SESSION['ADMIN_USERNAME']){
	...
	function clipboard($interface=false){
		header("Content-Type:text/html;charset=UTF-8");
		$sid = get_request('sid');
		if($_SESSION['ADMIN_LEVEL']==0){
			if($row=$this->rdp_set->select_by_id($sid)){
		...
		if($interface){
			return file_get_contents($sessions['clipboarddir']);
		}
		if(empty($logfile)){
