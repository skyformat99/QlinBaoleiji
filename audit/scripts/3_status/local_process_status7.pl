...
our $fd_lock;
our $lock_file = "/tmp/.local_process_status7";
...
our $time_date_str = "$year-$mon-$mday $hour:$min:00";
...
sub mail_alarm_process
{
    ...
    my $sqr_select = $dbh->prepare("select email from member where username='admin';");
    $sqr_select->execute();
    my $ref_select = $sqr_select->fetchrow_hashref();
    ...
    {
        print "no admin email in member\n";
        return
    }

    $sqr_select = $dbh->prepare("select datetime,type,context from local_status_warning_log where mail_status = -1");
    $sqr_select->execute();
    while($ref_select = $sqr_select->fetchrow_hashref())
    {
        ...
        unless(exists $email_errlog{$datetime})
        {
            $email_errlog{$datetime} = "$context\n";
        }
        else
        {
            $email_errlog{$datetime} .= "$context\n";
        }
        $device_status{$type} = 1;
    }
    $sqr_select->finish();

    my $subject = "localhost 主机监控告警 $time_date_str";
    my $alert_str = "";
    foreach my $datetime(keys %email_errlog)
    {
        $alert_str .= "$datetime 告警:\n$email_errlog{$datetime}\n";
    }

    if(length($alert_str) == 0)
    {
        return;
    }

    my $sqr_insert = $dbh->prepare("insert into mail_sender (mailto,subject,msg,program) values ('$email','$subject','$alert_str','local_process_status')");
    $sqr_insert->execute();
    $sqr_insert->finish();

    $sqr_select = $dbh->prepare("select max(id) from mail_sender");
    $sqr_select->execute();
    $ref_select = $sqr_select->fetchrow_hashref();
    ...
    system("/home/wuxiaolong/mail_sender.pl -i $id");

    $sqr_select = $dbh->prepare("select status from mail_sender where id=$id");
    $sqr_select->execute();
    $ref_select = $sqr_select->fetchrow_hashref();
    my $status = $ref_select->{"status"};
    $sqr_select->finish();

    my $sqr_update = $dbh->prepare("update local_status_warning_log set mail_status=$status+1");
    $sqr_update->execute();
    $sqr_update->finish();
    if($status == 0)
    {
        my $types = join(",", keys %device_status);
        print "update local_status set mail_last_sendtime = '$time_now_str' where type in ($types)\n";
        $sqr_update = $dbh->prepare("update local_status set mail_last_sendtime = '$time_now_str' where type in ($types)");
        $sqr_update->execute();
        $sqr_update->finish();
    }
}
