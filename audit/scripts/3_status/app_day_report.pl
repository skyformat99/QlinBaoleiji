...
sub insert_table
{
    ...
    my $sqr_insert = $dbh->prepare("insert into app_day_report(device_ip,port,date,app_name,app_type,avg,noval) values('$device_ip',$port,'$date_now_str','$app_name','$app_type',$avg,$nan_num)");
	$sqr_insert->execute();
	$sqr_insert->finish();
}
