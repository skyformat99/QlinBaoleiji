...
our $sqr_select = $dbh->prepare("select * from snmp_day_status where date >= now()-interval 1 week and date < now()");
$sqr_select->execute();
...
foreach my $key(keys %device_value)
{
	...
	if($type eq 'disk')
	{
		my $sqr_insert = $dbh->prepare("insert into snmp_week_status (device_ip,date_start,date_end,week_num,type,avg_value,max_value,disk) values ('$device_ip','$date_start',(CURDATE()-interval 1 day),DATE_FORMAT('$date_start','%v'),'$type',$avg_value,$max_value,'$disk')");
		$sqr_insert->execute();
		$sqr_insert->finish();
	}
	else
	{
		my $sqr_insert = $dbh->prepare("insert into snmp_week_status (device_ip,date_start,date_end,week_num,type,avg_value,max_value) values ('$device_ip','$date_start',(CURDATE()-interval 1 day),DATE_FORMAT('$date_start','%v'),'$type',$avg_value,$max_value)");
		$sqr_insert->execute();
		$sqr_insert->finish();
	}
}

my $sqr_delete = $dbh->prepare("delete from snmp_status where datetime <= now()-interval 1 week");
$sqr_delete->execute();
$sqr_delete->finish();
