...
sub fork_process
{
	...
	if ($pid == 0)
	{
		...
		if($temp->[1] eq 'cpu')
		{
			...
			&insert_table($dbh,$temp->[0],undef,'snmp_day_report_cpu',@results);
		}
		elsif($temp->[1] eq 'memory')
		{
			my @results = &rrd_fetch($temp->[3],'val');
			&insert_table($dbh,$temp->[0],undef,'snmp_day_report_memory',@results);
		}
		elsif($temp->[1] eq 'swap')
		{
			my @results = &rrd_fetch($temp->[3],'val');
			&insert_table($dbh,$temp->[0],undef,'snmp_day_report_swap',@results);
		}
		elsif($temp->[1] eq 'disk')
		{
			my @results = &rrd_fetch($temp->[3],'val');
			&insert_table($dbh,$temp->[0],$temp->[2],'snmp_day_report_disk',@results);
		}
...
sub insert_table
{
	my($dbh,$device_ip,$disk,$table_name,$avg,$max,$min,$nan_num) = @_;
