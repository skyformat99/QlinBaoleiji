...
my $sqr_select_count = $dbh->prepare("select server,user,srcip,protocol,status from log_login_day_count where date>=CURDATE()-interval 1 month and date<CURDATE() group by server,user,srcip,protocol,status");
...
while(my $ref_select_count = $sqr_select_count->fetchrow_hashref())
{
	...
	my $sql_insert_format = "insert into log_login_month_count (date_end";
